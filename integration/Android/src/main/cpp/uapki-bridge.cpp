...
#define UAPKI_LOG_DEBUG(...) __android_log_print(ANDROID_LOG_DEBUG, "UAPKI-DEV", __VA_ARGS__)
#else
#define UAPKI_LOG_DEBUG(...)
#endif

static const char* const STR_JNI_ERROR = R"({"errorCode":8193,"method":"","result":{},"error":"JNI_ERROR"})";

jboolean UapkiBridge::load (JNIEnv* env, jstring className, jint sleepMs)
{
    UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::load(env=%p, className, sleepMs)", env);
    ...
    UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::load(env, className='%s', sleepMs=%d)", m_ClassName.c_str(), m_SleepMs);
    if (!m_LibUapki.load()) return JNI_FALSE;

    m_SetJni = (f_uapki_bridge_setjni) DL_GET_PROC_ADDRESS(m_LibUapki.getHandle(), "set_jni");
    UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::load(), set m_SetJni=%p", m_SetJni);
...
{
    UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::process(env=%p, jsonRequest), m_SetJni=%p", env, m_SetJni);
    ...
    UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::process(env, jsonRequest='%s')", s_jsonreq);
    if (s_jsonreq) {
        char* s_jsonresult = m_LibUapki.process(s_jsonreq);
        UAPKI_LOG_DEBUG(ANDROID_LOG_DEBUG, "UAPKI-DEV", "UapkiBridge::process(env, jsonRequest), result=%s", s_jsonresult);
        if (s_jsonresult) {
